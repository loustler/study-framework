			...
			for (var n_current_wday = 0; n_current_wday < 7; n_current_wday++) {
				...
				if (dt_current_day.getMonth() == dt_datetime.getMonth()) {
					...
					str_buffer += "<a href=\"javascript:window.opener."
							...
							+ ".value='"
							+ tuning_date(dt_current_day)
							+ "'+document.cal.time.value; window.close();\"></a>";
				}
				str_buffer += "</td>\n";
...
function tuning(dt_datetime) {
	var mo = "";
	var dy = "";
	if ((dt_datetime.getMonth() + 1).toString().length == 1)
		mo = "0" + (dt_datetime.getMonth() + 1).toString();
	else
		mo = (dt_datetime.getMonth() + 1).toString();
	if (dt_datetime.getDate().toString().length == 1)
		dy = "0" + dt_datetime.getDate().toString();
	else
		dy = dt_datetime.getDate().toString();
