...
<%
	...
	if(request.getAttribute("purchaseVO")!=null) {
		purchaseVO = (PurchaseVO)request.getAttribute("purchaseVO");
	} else {
		System.out.println("updatePurchaseView.jsp VO not exist");
	}
	...
	<tr>
		...
		<td class="ct_write01">
			<select 	name="paymentOption" 	class="ct_input_g" style="width: 100px; height: 19px" 
							maxLength="20">
				<option value="000" <%= purchaseVO.getPaymentOption().equals("현금결재") ? "selected=\"selected\"" : "" %>>현금구매</option>
				<option value="001" <%= purchaseVO.getPaymentOption().equals("신용카드결재") ? "selected=\"selected\"" : "" %>>신용구매</option>
			</select>
		</td>
	</tr>
	<tr>
