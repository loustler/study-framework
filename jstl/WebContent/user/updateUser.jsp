	...
	<form name="detailForm" method="post">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top: 13px;">
			...
			<tr>
				...
				<td bgcolor="D6D6D6" width="1"></td>
				<td class="ct_write01">
					<u:forTokens var="i" items="${user.getPhone()}" delims="-" varStatus="status">

						<u:if test="${status.index == 0 }">
							<select name="phone1" class="ct_input_g" style="width: 50px; height: 20px" onChange="document.detailForm.phone2.focus();">
								<option value="010" ${i=='010' ? "selected" : ""}>010</option>
								<option value="011" ${i=='011' ? "selected" : ""}>011</option>
								<option value="016" ${i=='016' ? "selected" : ""}>016</option>
								<option value="018" ${i=='018' ? "selected" : ""}>018</option>
								<option value="019" ${i=='019' ? "selected" : ""}>019</option>
							</select>
						</u:if>

						<u:if test="${status.index == 1 }">
							<input type="text" name="phone2" value="${empty i ? i : i}" class="ct_input_g" style="width: 100px; height: 19px" maxLength="9" /> - 
						</u:if>

						<u:if test="${status.index == 2 }">
							<input type="text" name="phone3" value="${empty i ? i : i}" class="ct_input_g" style="width: 100px; height: 19px" maxLength="9" />
						</u:if>
					</u:forTokens>
